...
inline void AngleAxisToQuaternion(const T *angle_axis, T *quaternion) {
    ...
    if (theta_squared > T(std::numeric_limits<double>::epsilon())) {
        ...
        const T k = sin(half_theta) / theta; // sin(theta/2) / theta
        quaternion[0] = cos(half_theta); // q0 = cos(theta/2)
        quaternion[1] = a0 * k;
...
inline void QuaternionToAngleAxis(const T *quaternion, T *angle_axis) {
    ...
    // For quaternions representing non-zero rotation,
    // the conversion is numerically stable
    if (sin_squared_theta > T(std::numeric_limits<double>::epsilon())) {
        const T sin_theta = sqrt(sin_squared_theta);
        const T &cos_theta = quaternion[0];

        /**
         * if cos_theta is negative, theta is greater than pi/2,
         * which means that angle for the angle_axis vector
         * which is 2 * theta would be greater than pi ...
         */
         ...
         * computer correctly when Jets are used..
         */
        const T k(2.0);
        angle_axis[0] = q1 * k;
        angle_axis[1] = q2 * k;
...
inline void AngleAxisRotatePoint(const T angle_axis[3], const T pt[3], T result[3]) {
    ...
    if (theta2 > T(std::numeric_limits<double>::epsilon())) {
         ...
         *             w (w.pt) (1 - costheta)
         *
         * we want to be careful to only evaluate thesquare root
         * if the norm of the angle_axis vector is greater than zero.
         ...
         * matrix R corresponding to a vector w ang angle w is
         *
         *   R = I + hat(w) * sin(theta)
         *
